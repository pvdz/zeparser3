...
function ZeParser(code, goalMode = GOAL_SCRIPT, collectTokens = COLLECT_TOKENS_NONE, options = {}) {
  ...
  let tok_nextTokenGeneric = tok.nextTokenGeneric;
  let tok_nextTokenToParenOpen = tok.nextTokenToParenOpen;
  let tok_nextTokenToCurlyOpen = tok.nextTokenToCurlyOpen;
  let tok_nextTokenToParenOpenCurlyOpen = tok.nextTokenToParenOpenCurlyOpen;
  let tok_nextTokenToFrom = tok.nextTokenToFrom;
  let tok_nextTokenToString = tok.nextTokenToString;
  let tok_nextTokenToIdent = tok.nextTokenToIdent;
  let tok_nextTokenToArrow = tok.nextTokenToArrow;
  let tok_nextTokenToAs = tok.nextTokenToAs;
  let tok_nextTokenToAsCommaCurlyClose = tok.nextTokenToAsCommaCurlyClose;
  let tok_nextTokenToAsCommaFrom = tok.nextTokenToAsCommaFrom;
  let tok_nextTokenToColon = tok.nextTokenToColon;
  let tok_nextTokenToTarget = tok.nextTokenToTarget;
  let tok_nextTokenToStatementStart = tok.nextTokenToStatementStart;
  let tok_nextTokenToExpressionStart = tok.nextTokenToExpressionStart;
  let tok_nextTokenToExpressionStartGrouped = tok.nextTokenToExpressionStartGrouped;
  let tok_nextTokenToExpressionStartSemi = tok.nextTokenToExpressionStartSemi;
  let tok_nextTokenToExpressionStartSquareCloseComma = tok.nextTokenToExpressionStartSquareCloseComma;
  let tok_nextTokenToObjectMemberStart = tok.nextTokenToObjectMemberStart;
  let tok_nextTokenToObjectMemberRest = tok.nextTokenToObjectMemberRest;
  let tok_nextTokenToClassMemberStart = tok.nextTokenToClassMemberStart;
  let tok_nextTokenToClassMemberRest = tok.nextTokenToClassMemberRest;
  ...
  let tok_nextTokenToIdentCurlyClose = tok.nextTokenToIdentCurlyClose;
  let tok_nextTokenToIdentCurlyOpen = tok.nextTokenToIdentCurlyOpen;
  ...
  function skipRex(lexerFlags) {
    ...
    ASSERT(typeof curtok.c === 'number' && curtok.c >= 0 && curtok.c <= 0x10ffff, 'valid c', curtok);
  }
  ...
  function skipAny(lexerFlags) {
    ...
    ASSERT(curtok.c !== $$FWDSLASH_2F || (failForRegexAssertIfPass = curtok, regexAssertTrace = new Error().stack));
  }
  ...
  function ASSERT_skipAny(what, lexerFlags) {
    ASSERT(false, 'this function is a stub; a more direct lexer method should be called with a hint of what might follow next');
  }
  ...
  function skipForBabel(lexer, lexerFlags) {
    ...
    let maybeComment = curtok;
    while (isCommentToken(curtok.type)) {
      updateToken(lexer(lexerFlags));
      AST_babelAddComment(maybeComment);
      maybeComment = curtok;
    }
  ...
  function skipRexOrDieSingleChar(ord, lexerFlags) {
    ...
    ASSERT(typeof lexerFlags === 'number', 'lexerFlags number');
    if (curtok.c !== ord || curtok.str.length !== 1) {
      THROW('Next ord should be ' + ord + ' (`' + String.fromCharCode(ord) + '`) but was ' + curtok.c + ' (curc: `' + String.fromCharCode(curtok.c) + '`, token: `'+curtok.str+'`)');
    } else {
  ...
  function skipAnyOrDieSingleChar(ord, lexerFlags) {
    ...
    skipDivOrDieSingleChar(ord, lexerFlags);
    ASSERT(curtok.c !== $$FWDSLASH_2F || (failForRegexAssertIfPass = curtok, regexAssertTrace = new Error().stack));
  }
    ...
    switch (curtok.str) {
      ...
      case 'await':
        if (goalMode === GOAL_MODULE || hasAnyFlag(lexerFlags, LF_IN_ASYNC)) {
          ...
          ASSERT_skipDiv('await', lexerFlags); // TODO: bin-op or tail-op or asi-continuation or `)` `]` `}`
        }
        break;
      case 'yield':
        if (hasAnyFlag(lexerFlags, LF_IN_GENERATOR | LF_STRICT_MODE)) {
          ...
          ASSERT_skipRex($IDENT, lexerFlags); // Next is expr start or `*` or asi-continuation
        } else {
          ASSERT_skipDiv('yield', lexerFlags); // TODO: bin-op or tail-op or asi-continuation or `)` `]` `}`
        }
        break;
      default:
  ...
  function skipToParenOpen(lexerFlags) {
    // The next token must be a paren, possibly preceded by some whitespace
    // If it's not a paren then it falls back to the regular lexer. This function will validate the string afterwards.
    if (babelCompat) {
    ...
    ASSERT(curtok.type === $PUNCTUATOR, 'this token should be a punc');
  }
  ...
  function skipToFrom(lexerFlags) {
    ...
    if (curtok.str !== 'from') {
      THROW('Next token should be the ident `from` but was `' + curtok.str +'`');
    }
  ...
  function skipIdentStarCurlyOpenParenOpenString(lexerFlags) {
    ...
    // Since the rest has to check it anyways we don't need to validate it here
    ASSERT_VALID( isIdentToken(curtok.type) || curtok.str === '*' || curtok.str === '{' || curtok.str === '(' || isStringToken(curtok.type), 'not many options, wanted ident string * { (', curtok);
  }
  ...
  function parseBodyPartsWithDirectives(lexerFlags, scoop, labelSet, exportedNames, exportedBindings, paramsSimple, dupeParamErrorToken, functionNameTokenToVerify, isGlobalToplevel, fdState, astProp) {
    ...
    while (curtok.type !== $EOF && curtok.c !== $$CURLY_R_7D) {
  ...
  function parseStatementHeader(lexerFlags, headProp) {
    ...
    ASSERT_VALID(curtok.str === '(', 'all callers should have called `skipToParenOpen()` which should verify this invariant');
    ...
    if (curtok.c !== $$PAREN_R_29) THROW('Missing closing paren of statement header, found `' + curtok.str + '` instead');
  ...
  function parseSemiOrAsi(lexerFlags) {
    ...
    if (curtok.c === $$SEMI_3B) {
      ...
      if (curtok.c === $$CURLY_R_7D || curtok.nl > 0 || curtok.type === $EOF) {
        tok_asi();
      } else {
  ...
  function parseFunctionDeclaration(lexerFlags, scoop, isFuncDecl, isRealFuncExpr, asyncToken, functionToken, optionalIdent, isLabelled, fdState, astProp) {
    ...
    /*
     ...
     o={async foo(){}}
    */
    ASSERT_skipIdentStarParenOpen('function', lexerFlags); // TODO: why is this called for an async generator expression? maybe rename this func?
  ...
  function parseFunctionAfterKeyword(
    ...
    */
    ...
    if (curtok.type === $IDENT) {
      ...
      ASSERT_skipToParenOpen($IDENT, lexerFlags);
  ...
  function parseFunctionFromParams(lexerFlags, scoop, bindingFrom, expressionState, isClassConstructor, functionNameTokenToVerify, isMethod, asyncToken, starToken, getToken, setToken) {
    ...
    ASSERT(curtok.str === '{', 'the paretocurlyopen should already throw (not assert) if the next token is not a curly open');
  ...
  function parseFuncArguments(lexerFlags, scoop, bindingFrom, asyncToken, starToken, getToken, setToken) {
    ...
    if (curtok.c === $$PAREN_R_29) {
      ...
      ASSERT_skipToCurlyOpen(')', lexerFlags);
    }
      ...
      AST_destruct('params');
      ASSERT(curtok.c !== $$COMMA_2C, 'the trailing func comma case should already be caught by now');
      if (curtok.str !== ')') THROW('Missing function param definition closing parenthesis, found `' + curtok.str + '` instead');
      ASSERT_skipToCurlyOpen(')', lexerFlags);
    }
  ...
  function parseFunctionBody(lexerFlags, scoop, labelSet, blockType, paramsSimple, dupeParamErrorToken, functionNameTokenToVerify) {
    ...
    ASSERT(typeof lexerFlags === 'number');
    ASSERT(typeof labelSet === 'object');
    ASSERT_VALID(curtok.str === '{', 'block opening token should already have been confirmed, but not yet consumed');
    ASSERT([PARAMS_SOME_COMPLEX, PARAMS_SOME_NONSTRICT, PARAMS_ALL_SIMPLE].includes(paramsSimple), 'paramsSimple enum', paramsSimple);
    ASSERT_LABELSET(labelSet);
    ...
    if (curtok.c !== $$CURLY_R_7D) THROW('Missing function body closing curly, found `' + curtok.str + '` instead');
  ...
  function parseIdentStatement(lexerFlags, scoop, labelSet, exportedNames, exportedBindings, isGlobalToplevel, isLabelled, fdState, nestedLabels, astProp) {
    ...
    switch (curtok.str) {
      case 'async':
        ...
        if (curtok.c === $$COLON_3A) return parseLabeledStatementInstead(lexerFlags, scoop, labelSet, identToken, fdState, nestedLabels, astProp);
  ...
  function parseBlockStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    ASSERT(typeof lexerFlags === 'number', 'lexerFlags number');
    ASSERT(typeof lexerFlags === 'number');
    ASSERT(curtok.str === '{', 'should have thrown if curtok.c wasnt a curly when calling this function');
    ASSERT_LABELSET(labelSet);
  ...
  function parseBreakStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    // Note: must check eof/semi as well otherwise the value would be mandatory and parser would throw
    if (curtok.type === $IDENT && curtok.nl === 0) {
  ...
  function parseConstStatement(lexerFlags, scoop, isLabelled, fdState, astProp) {
    ...
    ASSERT_skipBindingStart('const', lexerFlags);
  ...
  function parseDoStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    parseNestedBodyPart((curtok.c !== $$CURLY_L_7B ? lexerFlags : lexerFlags) | LF_IN_ITERATION, scoop, labelSet, NOT_LABELLED, FDS_ILLEGAL, PARENT_NOT_LABEL, 'body');
    if (curtok.str !== 'while') THROW('A `do` must be followed by a `while`, but found `' + curtok.str + '` instead');
  ...
  function parseExportStatement(lexerFlags, scoop, exportedNames, exportedBindings, isGlobalToplevel, fdState, astProp) {
    ...
    ASSERT_skipIdentStarCurlyOpen('export', lexerFlags);

    let needsSemi = true; // only classes and function decls don't get this. var/let/const semi's are parsed elsewhere

    if (curtok.c === $$D_64 && curtok.str === 'default') {
      AST_openCustom(astProp, {
        type: 'ExportDefaultDeclaration',
        loc: AST_getBaseLoc(exportToken),
        declaration: undefined,
      });
      ASSERT_skipExpressionStart('default', lexerFlags); // Note: or a declaration, but that would start with ident, so is subsumed here... (footgun in 3... 2... 1...)
      ...
      if (curtok.str === 'class') {
      ...
      else if (curtok.c === $$F_66 && curtok.str === 'function') {
      ...
      else if (curtok.c === $$A_61 && curtok.str === 'async') {
        ...
        let identToken = curtok;
        ASSERT_skipRex('async', lexerFlags); // function, arrow, or value-tail (for it can be a legacy async)
      ...
      if (curtok.c === $$CURLY_L_7B) {
        ...
        if (curtok.str === 'from') {
          ...
          if (!isStringToken(curtok.type)) THROW('Export source must be a string');
      ...
      else if (curtok.c === $$V_76 && curtok.str === 'var') {
        // export var <bindings>
        let varToken = curtok;
        ASSERT_skipBindingStart('var' , lexerFlags);
      ...
      else if (curtok.c === $$L_6C && curtok.str === 'let') {
        // export let <bindings>
        let letToken = curtok;
        ASSERT_skipBindingStart('let' , lexerFlags);
  ...
  function parseExportObject(lexerFlags, tmpExportedNames, tmpExportedBindings) {
    ...
    while (curtok.type === $IDENT) {
      ...
      if (curtok.c === $$COMMA_2C) ASSERT_skipIdentCurlyClose(',', lexerFlags);
      else if (curtok.c !== $$CURLY_R_7D) THROW('Unexpected token while parsing export object');
    }
    ASSERT_VALID(curtok.c === $$CURLY_R_7D || curtok.str !== '}', 'curly close must be punc');
    if (curtok.c !== $$CURLY_R_7D) {
  ...
  function parseForStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    */
    ...
    let forToken = curtok;
    ASSERT_skipAwaitParenOpen('for', lexerFlags); // TODO: next must be `(` or `await`
    ...
    if (awaitable) {
      ...
      ASSERT_skipToParenOpen('await', lexerFlags);
    } else if (curtok.c !== $$PAREN_L_28) {
      THROW('Missing opening paren of the `for` header, found `' + curtok.str + '` instead');
    }
    ASSERT_skipExpressionStartSemi('(', lexerFlags); // TODO: expr start includes var/let/const as idents, also: `for (;;);`
    parseForHeader(sansFlag(lexerFlags | LF_NO_ASI, LF_IN_GLOBAL | LF_IN_SWITCH | LF_IN_ITERATION), forToken, scoop, awaitable, astProp);
    if (curtok.c !== $$PAREN_R_29) {
      THROW('Missing closing paren of the `for` header, found `' + curtok.str + '` instead');
    }
    ASSERT_skipStatementStart(')', lexerFlags);
  ...
  function parseForHeader(lexerFlags, forToken, scoop, awaitable, astProp) {
    ...
    if (curtok.type === $IDENT) {
      ...
      switch (curtok.str) {
        ...
        case 'let':
          ...
          // [v]: `for (let x of y);`
          //                ^
          if (curtok.type === $IDENT || curtok.c === $$SQUARE_L_5B || curtok.c === $$CURLY_L_7B) {
            // [v]: `for (let x of y);`
            //                ^
            // [v]: `for (let [x] in y);`
            //                ^
        ...
        case 'const':
          ...
          // - `for (const x;;);`
          let constToken = curtok;
          ASSERT_skipBindingStart('const' , lexerFlags); // TODO: next must be ident or destructuring [{
        ...
        default:
          ...
          hadAssign = curtok.c === $$IS_3D && curtok.str === '=';
    ...
    if (curtok.type === $IDENT) {
      ...
      if (curtok.str === 'in') {
        ...
        ASSERT_skipExpressionStart('in', lexerFlags);
        // `for (a in b=c) ..`
    ...
    let hadComma = curtok.c === $$COMMA_2C;
    if (hadComma) {
      // - `for (a, b;;);`
      //          ^
      // - `for (a, b in c);`
    ...
    if (curtok.c !== $$SEMI_3B) {
      if (hadComma && (curtok.str === 'of' || curtok.str === 'in')) {
      ...
      THROW('Missing first semi in `for` header, found `' + curtok.str + '` instead');
    }
    ...
    ASSERT_skipExpressionStartSemi(';', lexerFlags);
    ...
    if (curtok.c === $$SEMI_3B) {
      ...
      if (curtok.c !== $$SEMI_3B) {
        THROW('Missing second semi in `for` header, found `' + curtok.str + '` instead');
      }
    }

    ASSERT_skipExpressionStartGrouped(';', lexerFlags); // the second semi may immediately be followed by `)`
  ...
  function parseIfStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    if (curtok.type === $IDENT && curtok.str === 'else') {
      ASSERT_skipStatementStart('else', lexerFlags);
  ...
  function parseImportDeclaration(lexerFlags, scoop, isGlobalToplevel, fdState, astProp) {
    ...
    // TODO: use ASSERT_skipToParenOpen in non-global. In global it can be an import declaration so we can't do it there
    ASSERT_skipIdentStarCurlyOpenParenOpenString('import', lexerFlags);
    if (curtok.c === $$PAREN_L_28) {
    ...
    if (curtok.type === $IDENT) {
      ...
      if (curtok.c === $$COMMA_2C) {
        ...
        // import x, {...} from 'x'
        //         ^
        ASSERT_skipIdentStarCurlyOpen(',', lexerFlags);  // TODO: next must be `*` or `{` or ident
        if (curtok.c === $$STAR_2A) {
          // import x, * as z from 'x'
    ...
    if (!isStringToken(curtok.type)) THROW('Export source must be string');
  ...
  function parseImportObject(lexerFlags, scoop) {
    ...
    while (curtok.type === $IDENT) {
      ...
      if (curtok.c === $$COMMA_2C) ASSERT_skipIdentCurlyClose(',', lexerFlags);
      else if (curtok.c !== $$CURLY_R_7D) THROW('Unexpected character while parsing export object');
    }
    if (curtok.c !== $$CURLY_R_7D) THROW('Missing import definition closing curly, found `' + curtok.str + '` instead');
    // - `import {...} from 'x'`
    //               ^
    ...
    if (curtok.type === $IDENT) {
      ...
      if (identBindingErrorMsg === '') {
        ...
        if (hasAnyFlag(lexerFlags, LF_NO_ASI)) {
          THROW('`let` must be a declaration in strict mode but the next ident is a reserved keyword (`' + curtok.str + '`) and ASI does not apply here');
        }
        THROW('`let` must be a declaration in strict mode but the next ident is a reserved keyword (`' + curtok.str + '`)');
        ASSERT(false, 'unreachable');
      }
    } else if (curtok.c === $$SQUARE_L_5B || curtok.c === $$CURLY_L_7B) {
  ...
  function parseReturnStatement(lexerFlags, astProp) {
    ...
    if (curtok.nl > 0 && isRegexToken(curtok.type)) {
      ...
      if (curtok.nl === 0 && curtok.type !== $EOF && curtok.c !== $$SEMI_3B && curtok.c !== $$CURLY_R_7D) {
  ...
  function parseSwitchStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    if (curtok.c !== $$CURLY_L_7B) THROW('Missing opening curly of `switch` body, found `' + curtok.str + '` instead');
    ASSERT_skipSwitchBody($$CURLY_L_7B, lexerFlagsForSwitch);
    ...
    if (curtok.c !== $$CURLY_R_7D) THROW('Missing the closing curly of the switch body, found `' + curtok.str + '` instead');
    ASSERT_skipStatementStart('}', lexerFlags);
  ...
  function parseSwitchCases(lexerFlags, scoop, labelSet, astProp) {
    ...
    while (true) {
      if (curtok.str === 'case') {
        ...
        parseExpressions(lexerFlags, ASSIGN_EXPR_IS_OK, 'test');
        if (curtok.c !== $$COLON_3A) THROW('Missing colon after case expr');
        ASSERT_skipStatementStart(':', lexerFlags);
        while (curtok.type !== $EOF && curtok.c !== $$CURLY_R_7D && (curtok.type !== $IDENT || (curtok.str !== 'case' && curtok.str !== 'default'))) {
        ...
        if (curtok.c !== $$COLON_3A) THROW('Missing colon after default');
        ASSERT_skipStatementStart(':', lexerFlags);
        while (curtok.type !== $EOF && curtok.c !== $$CURLY_R_7D && (curtok.type !== $IDENT || (curtok.str !== 'case' && curtok.str !== 'default'))) {
          parseNestedBodyPart(lexerFlags, scoop, labelSet, NOT_LABELLED, FDS_LEX, PARENT_NOT_LABEL, 'consequent');
        }
  ...
  function parseTryStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    let hasEither = false;
    if (curtok.c === $$C_63 && curtok.str === 'catch') {
      // parseCatch
      hasEither = true;
      ...
      if (curtok.c === $$CURLY_L_7B) {
        ...
        ASSERT_skipBindingStart('(', lexerFlags); // Cannot be `)` so don't check for it
        if (curtok.c === $$PAREN_R_29) THROW('The catch clause must have a binding');
        ...
        if (curtok.c === $$COMMA_2C) THROW('Catch clause requires exactly one parameter, not more (and no trailing comma)');
        if (curtok.c === $$IS_3D && curtok.str === '=') THROW('Catch clause parameter does not support default values');
        if (curtok.c !== $$PAREN_R_29) THROW('Missing right paren for the catch clause, found `' + curtok.str + '` instead');
        ASSERT_skipToCurlyOpen(')', lexerFlags);
      } else {
        THROW('Missing start of catch clause (`(`) or start of catch body (`{`), found `' + curtok.str + '` instead')
      }

      ASSERT(curtok.c === $$CURLY_L_7B, 'should have thrown if curtok.c is not curly at this point');
    ...
    if (curtok.c === $$F_66 && curtok.str === 'finally') {
      // parseFinally
      hasEither = true;
      ASSERT_skipToCurlyOpen('finally', lexerFlags);
  ...
  function parseDeleteParenSpecialCase(lexerFlags, astProp){
    ...
    ASSERT(curtok.c === $$PAREN_L_28, 'this is why we are here');
    ...
    ASSERT_skipExpressionStartGrouped('(', lexerFlags); // `delete (/x/.y)`, for bonus points
    while (curtok.c === $$PAREN_L_28) {
      ++parens;
      parenToken = curtok;
      pees.push(curtok);
    ...
    while (--parens > 0) { // this only passes for inner parens
      ...
      if (curtok.c !== $$PAREN_R_29) {
        ...
        if (curtok.c === $$COMMA_2C) assignable = _parseExpressions(lexerFlags, openParenToken, assignable, astProp);
        canBeErrorCase = false;
      }
      // at least one rhs paren must appear now
      skipDivOrDieSingleChar($$PAREN_R_29, outerLexerflags); // TODO: next has too many options
      if (curtok.str === '=>') {
  ...
  function parseLabeledStatementInstead(lexerFlags, scoop, labelSet, identToken, fdState, nestedLabels, astProp) {
    ...
    if (labelName !== 'eval' && labelName !== 'arguments') {
  ...
  function parseAnyVarDeclaration(lexerFlags, bindingToken, scoop, bindingType, bindingOrigin, exportedNames, exportedBindings, astProp) {
    ...
    if (curtok.type !== $IDENT && curtok.c !== $$SQUARE_L_5B && curtok.c !== $$CURLY_L_7B) THROW('Expected identifier, or array/object destructuring, next token is: ' + curtok);
  ...
  function parseBindings(lexerFlags, scoop, bindingType, bindingOrigin, defaultOptions, setToken, exportedNames, exportedBindings, astProp) {
    ...
    let many = 0;
    let inited = false;
    let startWasObjectOrArray = curtok.c === $$SQUARE_L_5B || curtok.c === $$CURLY_L_7B;
    let paramsSimple = PARAMS_ALL_SIMPLE;
    do {
      ++many;
      let bindingStartToken = curtok;
      let wasRest = curtok.c === $$DOT_2E && bindingStartToken.str === '...';
      ...
      if (wasRest) {
        ASSERT(curtok.c === $$PAREN_R_29, 'the "rest is last and no init" check should happen elsewhere and before this point');
        break;
      }
      if (curtok.c !== $$COMMA_2C) break;
      ASSERT_skipBindingStartGrouped(',', lexerFlags);
      if (curtok.c === $$PAREN_R_29) {
        // `function f(a,)`
  ...
  function parseBinding(lexerFlags, bindingStartToken, scoop, bindingType, bindingOrigin, defaultsOption, exportedNames, exportedBindings, astProp) {
    ...
    ASSERT(defaultsOption === ASSIGNMENT_IS_INIT || defaultsOption === ASSIGNMENT_IS_DEFAULT, 'enum');
    ASSERT_BINDING_TYPE(bindingType);
    ASSERT_BINDING_ORIGIN(bindingOrigin);
    ...
    else if (bindingType === BINDING_TYPE_CONST && (bindingOrigin !== FROM_FOR_HEADER || (curtok.c === $$SEMI_3B || curtok.c === $$COMMA_2C))) {
  ...
  function checkIdentReadable(lexerFlags, bindingType, identToken) {
    ...
    switch (identToken.canon) {
      ...
      case 'false':
        ...
        return true; // true if reading, false if binding
  ...
  function parseParenlessArrowAfterAsync(lexerFlags, fromStmtOrExpr, allowAssignment, asyncToken, astProp) {
    ...
    ASSERT_skipArrow($IDENT, lexerFlags); // this was `async <curtok>` and curtok is not a keyword
  ...
  function parseExpressionFromBinaryOp(lexerFlags, firstExprToken, assignable, astProp) {
    ...
    let first = true;
    while (isNonAssignBinOp(lexerFlags) || curtok.c === $$QMARK_3F) {
      if (curtok.c === $$QMARK_3F) {
  ...
  function parseExpressionFromTernaryOp(lexerFlags, firstExprToken, astProp) {
    ...
    if (curtok.c !== $$COLON_3A) {
      if (curtok.c === $$COMMA_2C) THROW('Can not use comma inside ternary expressions');
  ...
  function _parseExpressions(lexerFlags, startOfFirstExprToken, assignableForPiggies, astProp) {
    ...
    ASSERT(curtok.c === $$COMMA_2C, 'confirm at callsite');
  ...
  function parseValueHeadBody(lexerFlags, maybe, isNewArg, allowAssignment, leftHandSideExpression, astProp) {
    ...
    else if (curtok.type === $PUNCTUATOR) {
      if (curtok.c === $$CURLY_L_7B) {
  ...
  function parseSuperKeyword(lexerFlags, superToken, astProp) {
    ...
    if (curtok.c === $$SQUARE_L_5B || curtok.str === '.') {
      // super.foo
      // super[foo]
  ...
  function parseUnary(lexerFlags, isNewArg, astProp) {
    ...
    ASSERT_skipExpressionStart(unaryToken.str, lexerFlags); // next can be regex (`+/x/.y`), though it's very unlikely
  ...
  function parseYieldStarArgument(lexerFlags, allowAssignment, astProp) {
    ...
    ASSERT_skipExpressionStart('*', lexerFlags); // next is any value
  ...
  function parseYieldArgument(lexerFlags, astProp) {
    ...
    let hadValue = curtok.nl > 0 ? YIELD_WITHOUT_VALUE : parseYieldValueMaybe(lexerFlags, ASSIGN_EXPR_IS_OK, astProp);
  ...
  function parseValueTail(lexerFlags, valueFirstToken, assignable, isNewArg, leftHandSideExpression, astProp) {
    ...
    switch (curtok.c) {
      case $$DOT_2E:
        if (curtok.str === '.') return _parseValueTailDotProperty(lexerFlags, valueFirstToken, assignable, isNewArg, astProp);
        break;
      case $$SQUARE_L_5B:
      ...
      case $$TICK_60:
        if (isTemplateStart(curtok.type)) return _parseValueTailTemplate(lexerFlags, valueFirstToken, assignable, isNewArg, astProp);
        break;
      case $$PLUS_2B:
        if (isNewArg === IS_NEW_ARG) return _parseValueTailNewArg(assignable);
        if (curtok.str === '++') return _parseValueTailUpdate(lexerFlags, valueFirstToken, assignable, leftHandSideExpression, astProp);
        break;
      case $$DASH_2D:
        if (isNewArg === IS_NEW_ARG) return _parseValueTailNewArg(assignable);
        if (curtok.str === '--') return _parseValueTailUpdate(lexerFlags, valueFirstToken, assignable, leftHandSideExpression, astProp);
  ...
  function _parseValueTailCall(lexerFlags, valueFirstToken, assignable, isNewArg, astProp) {
    ASSERT_VALID(curtok.type === $PUNCTUATOR && curtok.str === '(');
  ...
  function _parseValueTailUpdate(lexerFlags, valueFirstToken, assignable, leftHandSideExpression, astProp) {
    ...
    ASSERT(curtok.str === '++' || curtok.str === '--', 'have not consumed the update op yet');
  ...
  function parseCallArgs(lexerFlags, astProp) {
    ASSERT_skipExpressionStartGrouped('(', lexerFlags);
    // [v]: `for (x(y in z);;);`
    lexerFlags = sansFlag(lexerFlags | LF_NO_ASI, LF_IN_FOR_LHS);
    ...
    if (curtok.c === $$PAREN_R_29) {
      ...
      skipDivOrDieSingleChar($$PAREN_R_29, lexerFlags); // TODO: next has too many options
    }
  ...
  function parseDynamicImport(lexerFlags, importToken, astProp) {
    ...
    ASSERT(curtok.c === $$PAREN_L_28, 'havent consumed the paren yet');
  ...
  function parseArrowFromPunc(lexerFlags, paramScoop, asyncToken, allowAssignment, paramsSimple) {
    ...
    ASSERT(asyncToken === UNDEF_ASYNC || asyncToken.str === 'async', 'async token');
    ASSERT_skipExpressionStart('=>', lexerFlags); // the `{` for a block is subsumed in the expr-start lexer hint
    ASSERT_ASSIGN_EXPR(allowAssignment);
...

    {
      ...
      if ((isAssignBinOp() || isNonAssignBinOp(lexerFlags)) && (curtok.nl === 0 || curtok.c === $$FWDSLASH_2F)) {
        // - `()=>{}+a'
  ...
  function parseGroupToplevels(lexerFlags, asyncStmtOrExpr, allowAssignment, asyncToken, newlineAfterAsync, leftHandSideExpression, astProp) {
    ...
    ASSERT(asyncToken === UNDEF_ASYNC || asyncToken.str === 'async', 'async token');
    ASSERT(curtok.str === '(', 'should have thrown if not currently at paren open');
    ASSERT_ASSIGN_EXPR(allowAssignment);
    let parenToken = curtok;

    ASSERT_skipExpressionStartGrouped('(', lexerFlags); // Patterns are subsumed by exprs when it comes to skip
  ...
  function _parseGroupToplevels(lexerFlagsBeforeParen, parenToken, asyncStmtOrExpr, allowAssignmentForGroupToBeArrow, isDeleteArg, asyncToken, newlineAfterAsync, leftHandSideExpression, astProp) {
    ...
    if (curtok.c === $$PAREN_R_29) {
      ...
      if (asyncToken !== UNDEF_ASYNC) {
        skipDivOrDieSingleChar($$PAREN_R_29, lexerFlags); // can be `async() / x`     TODO: next must be `=>` or non-binop expr tail or asi continuation
    ...
    while (curtok.c !== $$PAREN_R_29) { // top-level group loop, list of ident, array, object, rest, and other expressions
      if (curtok.type === $IDENT) {
        // - (x)
        // - (x = y)
        // - (x, y)
        ...
        let wasCommaOrEnd = curtok.c === $$COMMA_2C || curtok.c === $$PAREN_R_29;

        ASSERT(toplevelComma || curtok.c !== $$PAREN_R_29 || assignable === ASSIGNABLE_UNDETERMINED, 'for group with one simple element, delete edge case');
        ...
        if (wasAssignment) {
          ...
          if (!toplevelComma && curtok.c === $$PAREN_R_29) {
          ...
          // if curtok.c is a comma then the group is not assignable but that will fail through the toplevelComma flag
        ...
        if (curtok.c !== $$COMMA_2C && curtok.c !== $$PAREN_R_29) {
        ...
        skipDivOrDieSingleChar($$PAREN_R_29, lexerFlags); // TODO: next must be `=>` or non-binop expr continuation or asi-continuation
  ...
  function parseArrowableTopRest(lexerFlags, scoop, asyncToken, astProp) {
    ...
    let subDestruct = parseArrowableSpreadOrRest(lexerFlags, scoop, $$PAREN_R_29, BINDING_TYPE_ARG, asyncToken, UNDEF_EXPORTS, UNDEF_EXPORTS, astProp);
    if (asyncToken === UNDEF_ASYNC) {
      if (hasAllFlags(subDestruct, CANT_DESTRUCT) || curtok.c === $$COMMA_2C) {
        THROW('The ... argument must be destructible in an arrow header, found something that was not destructible');
      }
      if (curtok.c === $$IS_3D && curtok.str === '=') THROW('Cannot set a default on a rest value');
      if (curtok.c === $$COMMA_2C) THROW('Rest arg cannot have a trailing comma');
      if (curtok.c !== $$PAREN_R_29) THROW('Rest arg must be last but did not find closing paren');
    }
  ...
  function parseArrayLiteralPattern(lexerFlagsBeforeParen, scoop, bindingType, skipInit, exportedNames, exportedBindings, _astProp) {
    ...
    ASSERT(curtok.c === $$SQUARE_L_5B, 'not yet consumed');
    ASSERT_BINDING_TYPE(bindingType);
    ...
    while (curtok.c !== $$SQUARE_R_5D) {
      ...
      if (curtok.type === $IDENT) {
        ...
        let nextIsCommaOrEnd = curtok.c === $$COMMA_2C || curtok.c === $$SQUARE_R_5D;
        ...
        if (nextIsAssignment) {
          ...
          let nowDestruct = parseOptionalDestructibleRestOfExpression(lexerFlags, identToken, bindingType, leftAssignable, CANT_DESTRUCT, $$SQUARE_R_5D, astProp);
        ...
        destructible |= parseOptionalDestructibleRestOfExpression(lexerFlags, objOpenToken, bindingType, hasAllFlags(objDestructible, CANT_DESTRUCT) ? NOT_ASSIGNABLE : IS_ASSIGNABLE, objDestructible, $$SQUARE_R_5D, astProp);
      }
      else if (curtok.c === $$SQUARE_L_5B) {
        // - [[]]
        //    ^
        ...
        destructible |= parseOptionalDestructibleRestOfExpression(lexerFlags, arrOpenToken, bindingType, hasAllFlags(arrDestructible, CANT_DESTRUCT) ? NOT_ASSIGNABLE : IS_ASSIGNABLE, arrDestructible, $$SQUARE_R_5D, astProp);
      }
      else if (curtok.c === $$DOT_2E && curtok.str === '...') {
        ...
        let subDestruct = parseArrowableSpreadOrRest(lexerFlags, scoop, $$SQUARE_R_5D, bindingType, UNDEF_ASYNC, exportedNames, exportedBindings, astProp);
        destructible |= subDestruct;
        if (curtok.c !== $$COMMA_2C && curtok.c !== $$SQUARE_R_5D) {
          THROW('Encountered unexpected token after parsing spread/rest argument ');
        }

        ASSERT(curtok.c !== $$COMMA_2C || hasAllFlags(subDestruct, CANT_DESTRUCT), 'if comma then cannot destruct, should be dealt with in spread-parsing function');
        ...
        let wasParen = curtok.c === $$PAREN_L_28;
      ...
      if (curtok.c !== $$COMMA_2C) break; // end of the array
  ...
  function parseObjectLiteralPatternAndAssign(lexerFlags, scoop, bindingType, skipInit, exportedNames, exportedBindings, astProp) {
    ...
    ASSERT(skipInit === SKIP_INIT || skipInit === PARSE_INIT, 'skipInit is enum', skipInit);
    ASSERT(curtok.c === $$CURLY_L_7B, 'not yet consumed');
  ...
  function parseObjectLikePatternSansAssign(outerLexerFlags, scoop, bindingType, isExpression, exportedNames, exportedBindings, astProp) {
    ...
    ASSERT(curtok.c === $$CURLY_L_7B, 'should have asserted to be at the curly open of an objlit or pattern here...', curtok);
    ...
    ASSERT_skipObjectMemberStart('{', lexerFlags);
  ...
  function parseObjectLikePart(lexerFlags, scoop, bindingType, exportedNames, exportedBindings, astProp) {
    ...
    if (curtok.type === $IDENT) {
      ...
      // - (z = {...x.y}) => z        (ok)
      // - (z = {...x.y} = z) => z    (ok)
      let subDestruct = parseArrowableSpreadOrRest(lexerFlags, scoop, $$CURLY_R_7D, bindingType, UNDEF_ASYNC, exportedNames, exportedBindings, astProp);
      ASSERT(typeof subDestruct === 'number', 'should be number');
      destructible |= subDestruct;
      ASSERT(curtok.type === $EOF || (curtok.c !== $$COMMA_2C || hasAllFlags(subDestruct, CANT_DESTRUCT)), 'if comma then cannot destruct, should be dealt with in function');
      ASSERT(curtok.type === $EOF || (curtok.c === $$COMMA_2C || curtok.c === $$CURLY_R_7D), 'abstraction should parse whole rest/spread goal; ' + curtok);
    }
    else if (curtok.c === $$SQUARE_L_5B) {
      ...
      let starToken = curtok;
      ASSERT_skipIdentStringNumberSquareOpen('*', lexerFlags);
      ...
      if (curtok.type === $IDENT) {
        ...
        ASSERT_skipToParenOpen(litToken.str, lexerFlags);
        AST_setLiteral(astProp, litToken);
  ...
  function _parseObjectPropertyValueAfterColon(lexerFlags, keyToken, bindingType, assignableOnlyForYieldAwaitFlags, destructible, scoop, exportedNames, exportedBindings, astProp) {
    ...
    if (curtok.type === $IDENT) {
      ...
      // - `{key: bar, koo: baa}`
      // - `{[key]: bar}`
      let commaOrEnd = curtok.c === $$COMMA_2C || curtok.c === $$CURLY_R_7D;
      ...
      if (curtok.c === $$COMMA_2C || curtok.c === $$CURLY_R_7D) {
        // - `({a: b} = d)`
        //          ^
        // - `({a: b = x} = d)`
        //           ^
      ...
      destructible |= parseOptionalDestructibleRestOfExpression(lexerFlags, arrStartToken, bindingType, hasAllFlags(nowDestruct, CANT_DESTRUCT) ? NOT_ASSIGNABLE : IS_ASSIGNABLE, nowDestruct, $$CURLY_R_7D, 'value');
    }
    else if (curtok.c === $$CURLY_L_7B) {
      // - `({ident: {})`
      //             ^
      ...
      destructible |= parseOptionalDestructibleRestOfExpression(lexerFlags, objStartToken, bindingType, hasAllFlags(nowDestruct, CANT_DESTRUCT) ? NOT_ASSIGNABLE : IS_ASSIGNABLE, nowDestruct, $$CURLY_R_7D, 'value');
    }
    else {
      // - `{x: 15}`
      //        ^
  ...
  function parseObjectLikePartFromIdent(lexerFlags, startOfPropToken, scoop, bindingType, exportedNames, exportedBindings, astProp) {
    ...
    ASSERT_skipObjectMemberRest($IDENT, lexerFlags);

    if (allowAsyncFunctions) {
      // Note: `{async\n(){}}` is legal in sloppy so we do have to check the paren
      if (curtok.c !== $$PAREN_L_28 && curtok.nl > 0 && propLeadingIdentToken.str === 'async') {
        // - `{async \n key(){}}`
        //              ^
    ...
    if (curtok.c === $$COMMA_2C || curtok.c === $$CURLY_R_7D || curtok.str === '=') {
      ...
      asyncToken = propLeadingIdentToken;
      if (asyncToken.str !== 'async') { // TODO: should it read from canon?
        // - `{get *foo(){}}`
      ...
      if (curtok.type === $IDENT) {
        ...
        if (curtok.c !== $$SQUARE_R_5D) THROW('Missing right square bracket for computed property, found `' + curtok.str + '` instead');
        ASSERT_skipToParenOpen(']', lexerFlags);
  ...
  function parseClassId(lexerFlags, optionalIdent, scoop) {
    ...
    if (curtok.type === $IDENT && curtok.str !== 'extends') {
      ...
      let idToken = curtok;
      ASSERT_skipIdentCurlyOpen($IDENT, lexerFlags); // TODO: this could explicitly check for `extends` but I think this is fine
  ...
  function _parseClass(outerLexerFlags, originalOuterLexerFlags, scoop, isExpression) {
    ...
    if (curtok.type === $IDENT && curtok.str === 'extends') {
      ...
      ASSERT_skipExpressionStart('extends', outerLexerFlags);
    ...
    if (curtok.c !== $$CURLY_L_7B) THROW('Expected the opening curly `{` of a class body, found `' + curtok.str + '` instead');
  ...
  function _parseClassBody(lexerFlags, outerLexerFlags, originalOuterLexerFlags, scoop, bindingType, isExpression, exportedNames, exportedBindings, astProp) {
    ...
    // - `(class = x)`
    //           ^
    ASSERT_skipClassMemberStart('{', lexerFlags);
    while (curtok.c === $$SEMI_3B) {
      ASSERT_skipClassMemberStart(';', lexerFlags);
    }
    ...
    if (isExpression === IS_EXPRESSION) {
      ...
      ASSERT_skipStatementStart('}', originalOuterLexerFlags);
    }
  ...
  function parseClassMethod(lexerFlags, outerLexerFlags, scoop, bindingType, exportedNames, exportedBindings, astProp) {
    ...
    if (curtok.type === $IDENT && curtok.str === 'static') {
      ...
      staticToken = curtok;
      // = `class x { static / foo(){} }`
      ASSERT_skipClassMemberStart('static', lexerFlags); // The token after `static` can only be the same set of tokens valid at the start of a member (including `static` itself)
  ...
  function parseClassMethodFromIdent(lexerFlags, outerLexerFlags, scoop, bindingType, staticToken, exportedNames, exportedBindings, astProp) {
    ...
    let identToken = curtok;
    ASSERT_skipClassMemberRest($IDENT, lexerFlags);

    if (allowAsyncFunctions) {
      if (curtok.c !== $$PAREN_L_28 && curtok.nl > 0 && identToken.str === 'async') {
        // - `{async \n key(){}}`
        //              ^
        // Always an error due to async being a restricted production
        // Note that `{async(){}}` is legal so we must check the curtok.c
  ...
  function parseClassMethodComputedKey(lexerFlags, outerLexerFlags, staticToken, asyncToken, starToken, getToken, setToken, astProp) {
    ...
    // - `(fail = class A {[await](){}; "x"(){}}) => {}`
    if (curtok.c !== $$SQUARE_R_5D) THROW('Missing right square bracket for computed member, found `' + curtok.str + '` instead');
    ASSERT_skipToParenOpen(']', lexerFlags);
  ...
  function parseClassMethodAfterKey(lexerFlags, staticToken, asyncToken, starToken, getToken, setToken, keyToken, bracketOpenToken, astProp) {
    ...
    if (keyToken !== undefined && staticToken !== UNDEF_STATIC) {
      if (keyToken.type === $IDENT && keyToken.canon === 'prototype') {
        ...
        keyToken.canon.slice(1, -1) === 'prototype'
      ) {
        THROW('Static class methods can not be called `prototype`');
      }
    }
    ...
    // [v]: `class A {a(){}}`
    ASSERT(curtok.c === $$PAREN_L_28, 'these (non-assert) checks have already been applied at this point');
    parseFunctionAfterKeyword(
      lexerFlags,
      DO_NOT_BIND,
      NOT_FUNC_DECL,
  ...
  function parseOptionalDestructibleRestOfExpression(lexerFlags, valueStartToken, bindingType, assignable, destructible, closingCharOrd, astProp) {
    ...
    if (curtok.c === $$COMMA_2C || curtok.c === closingCharOrd) {
  ...
  function parseArrowableSpreadOrRest(lexerFlags, scoop, closingCharOrd, bindingType, asyncToken, exportedNames, exportedBindings, astProp) {
    // parseArrowableRest
    ...
    if (curtok.c === $$DOT_2E && curtok.str === '...') THROW('Can not rest twice');
    AST_openCustom(astProp, {
      type: 'SpreadElement',
      loc: AST_getBaseLoc(spreadToken),
      argument: undefined,
    });
    let destructible = _parseArrowableSpreadOrRest(lexerFlags, scoop, closingCharOrd, bindingType, asyncToken, exportedNames, exportedBindings, 'argument');
    AST_close('SpreadElement');

    if (curtok.c !== closingCharOrd && curtok.c !== $$COMMA_2C) THROW('Encountered invalid input after spread/rest argument');

    return destructible;
  }
  function _parseArrowableSpreadOrRest(lexerFlags, scoop, closingCharOrd, bindingType, asyncToken, exportedNames, exportedBindings, astProp) {
    ...
    if (curtok.type === $IDENT) {
      ...
      let willBeSimple = curtok.c === closingCharOrd || curtok.c === $$COMMA_2C || assignBefore;
      if (willBeSimple) {
      ...
      if (curtok.str !== '=' && curtok.c !== closingCharOrd && curtok.c !== $$COMMA_2C) {
        // - `({...[].x} = x);`
        destructible = parseOptionalDestructibleRestOfExpression(lexerFlags, argStartToken, bindingType, assignable, nowDestruct, closingCharOrd, astProp);
      } else {
    ...
    else if (curtok.c === closingCharOrd) {
      // `[...]`
      // `(...)`
  ...
  function parseObjectLikeMethodAfterKey(lexerFlags, asyncToken, starToken, getToken, setToken, keyToken, bracketOpenToken, astProp) {
    ...
    if (curtok.c !== $$PAREN_L_28) {
      ...
      // [x]: `{get 123: x}`
      // [x]: `{async foo: x}`
      THROW('Expected to parse a paren of the method now but found `' + curtok.str + '`');
