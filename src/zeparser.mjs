...
function ZeParser(code, goalMode = GOAL_SCRIPT, collectTokens = COLLECT_TOKENS_NONE, options = {}) {
  ...
  function THROW_TOKEN(desc, token, ...args) {
    ...
    tok_throw('Parser error! ' + desc + (curtype === $EOF ? ' (at EOF)' : ''), token, undefined, fullErrorContext);
  }
  ...
  function AST_setIdent(astProp, token) {
    ...
    AST_open(astProp, 'Identifier', token);
  ...
  function AST_setLiteral(astProp, token, fromDirective) {
    ...
    if (isStringToken(token.type)) {
      ...
      if (babelCompat) {
        ...
        AST_set('value', token.canon.slice(1, -1));
  ...
  function skipRex(lexerFlags) {
    ...
    ASSERT(typeof curc === 'number' && curc >= 0 && curc <= 0x10ffff, 'valid c', curtok);
  }
  ...
  function ASSERT_skipRex(what, lexerFlags) {
    ...
    if (typeof what === 'string') {
      ...
      if (ALL_TOKEN_GROUPS.includes(what)) {
        ...
        ASSERT((curtype & what) === what, 'Expected to parse a particular group type of token', what);
      } else {
  ...
  function skipToParenOpen(lexerFlags) {
    ...
    ASSERT(curtype === $PUNCTUATOR, 'this token should be a punc');
  }
  ...
  function skipToFrom(lexerFlags) {
    ...
    ASSERT(isIdentToken(curtype), 'a token whose value "from" should be an ident');
  }
  ...
  function skipStatementStart(lexerFlags) {
    ...
    ASSERT_VALID(
      isIdentToken(curtype) ||
      isStringToken(curtype) ||
      isTemplateStart(curtype) ||
      isNumberToken(curtype) ||
      isRegexToken(curtype) ||
      curtok.str === '!' ||
      curtok.str === '[' ||
      curtok.str === '{' ||
      curtok.str === '(' ||
      ...
      curtok.str === ';' ||
      curtok.str === '}' ||
      curtype === $EOF || // do-while at at eof. do while is not a problem otherwise... (I think)
  ...
  function skipIdentStringNumberSquareOpen(lexerFlags) {
    ...
    ASSERT_VALID( isIdentToken(curtok.type) || isStringToken(curtype) || isNumberToken(curtype) || curtok.str === '[', 'not many options, wanted ident number string [', curtok);
  }
  ...
  function parseStatementHeader(lexerFlags, headProp) {
    ...
    if (curc !== $$PAREN_R_29) THROW('Missing closing paren of statement header, found `' + curtok.str + '` instead');
  ...
  function parseNestedBodyPart(lexerFlags, scoop, labelSet, isLabelled, fdState, nestedLabels, astProp) {
    ...
    if (curtype === $EOF) THROW('Statement must have a sub-statement but found EOF instead');
  ...
  function parseBodyPart(lexerFlags, scoop, labelSet, exportedNames, exportedBindings, isGlobalToplevel, isLabelled, fdState, nestedLabels, astProp) {
    ...
    ASSERT(typeof lexerFlags === 'number', 'lexerFlags number');
    ASSERT(curtype !== $ERROR && curtype !== $EOF, 'token type should not have $error or $eof at this point');
  ...
  function parseFunctionBody(lexerFlags, scoop, labelSet, blockType, paramsSimple, dupeParamErrorToken, functionNameTokenToVerify) {
    ...
    if (curc !== $$CURLY_R_7D) THROW('Missing function body closing curly, found `' + curtok.str + '` instead');
  ...
  function parseIdentStatement(lexerFlags, scoop, labelSet, exportedNames, exportedBindings, isGlobalToplevel, isLabelled, fdState, nestedLabels, astProp) {
    ...
    switch (curtok.str) {
      case 'async':
        ...
        if (curc === $$COLON_3A) return parseLabeledStatementInstead(lexerFlags, scoop, labelSet, identToken, fdState, nestedLabels, astProp);
  ...
  function _parseFromLiteralStatement(lexerFlags, litToken, astProp) {
    ...
    AST_open(astProp, 'ExpressionStatement', litToken);
  ...
  function parseBlockStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    ASSERT(typeof lexerFlags === 'number', 'lexerFlags number');
    ASSERT(typeof lexerFlags === 'number');
    ASSERT(curtok.str === '{', 'should have thrown if curc wasnt a curly when calling this function');
    ASSERT_LABELSET(labelSet);
    ...
    if (curtype === $EOF) THROW('Unexpected EOF while parsing a block');
    ASSERT_skipStatementStart('{', lexerFlags);
    AST_close('BlockStatement');
  ...
  function parseDoStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    parseNestedBodyPart((curc !== $$CURLY_L_7B ? lexerFlags : lexerFlags) | LF_IN_ITERATION, scoop, labelSet, NOT_LABELLED, FDS_ILLEGAL, PARENT_NOT_LABEL, 'body');
    ...
    if (curc === $$D_64 && curtok.str === 'default') {
      ...
      if (curc === $$CURLY_L_7B) {
        ...
        if (curtok.str === 'from') {
          ...
          if (!isStringToken(curtype)) THROW('Export source must be a string');
  ...
  function parseExportObject(lexerFlags, tmpExportedNames, tmpExportedBindings) {
    ...
    ASSERT_VALID(curc === $$CURLY_R_7D || curtok.str !== '}', 'curly close must be punc');
    if (curc !== $$CURLY_R_7D) {
  ...
  function parseImportDeclaration(lexerFlags, scoop, isGlobalToplevel, fdState, astProp) {
    ...
    if (!isStringToken(curtype)) THROW('Export source must be string');
  ...
  function parseImportObject(lexerFlags, scoop) {
    ...
    if (curc !== $$CURLY_R_7D) THROW('Missing import definition closing curly, found `' + curtok.str + '` instead');
    // - `import {...} from 'x'`
    //               ^
    ASSERT_skipToFrom('}', lexerFlags);
  ...
  function parseSwitchStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    if (curc !== $$CURLY_L_7B) THROW('Missing opening curly of `switch` body, found `' + curtok.str + '` instead');
    ...
    if (curc !== $$CURLY_R_7D) THROW('Missing the closing curly of the switch body, found `' + curtok.str + '` instead');
    ASSERT_skipStatementStart('}', lexerFlags);
  ...
  function parseTryStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    if (curc === $$C_63 && curtok.str === 'catch') {
      ...
      if (curc === $$CURLY_L_7B) {
        ...
        if (curc === $$PAREN_R_29) THROW('The catch clause must have a binding');
        ...
        if (curc !== $$PAREN_R_29) THROW('Missing right paren for the catch clause, found `' + curtok.str + '` instead');
        ASSERT_skipToCurlyOpen(')', lexerFlags);
      } else {
        THROW('Missing start of catch clause (`(`) or start of catch body (`{`), found `' + curtok.str + '` instead')
      }

      ASSERT(curc === $$CURLY_L_7B, 'should have thrown if curc is not curly at this point');
  ...
  function parseAnyVarDeclaration(lexerFlags, bindingToken, scoop, bindingType, bindingOrigin, exportedNames, exportedBindings, astProp) {
    ...
    AST_open(astProp, 'VariableDeclaration', bindingToken);
  ...
  function parseBindings(lexerFlags, scoop, bindingType, bindingOrigin, defaultOptions, setToken, exportedNames, exportedBindings, astProp) {
    ...
    do {
      ...
      let wasRest = curc === $$DOT_2E && bindingStartToken.str === '...';
  ...
  function parseValueHeadBody(lexerFlags, maybe, isNewArg, allowAssignment, leftHandSideExpression, astProp) {
    ...
    if (curtype === $IDENT) {
    ...
    else if (isNumberStringRegex(curtype)) {
      let litToken = curtok;
      ...
      if (curc === $$CURLY_L_7B) {
        ...
        // `new.fail`
        // `(.fail)`
        ASSERT(curtype !== $NUMBER_DEC, 'should be checked elsewhere');
  ...
  function parseQuasiPart(lexerFlags, wasTail, allowBadEscapes) {
    ...
    if (curtype === $TICK_PURE || curtype === $TICK_TAIL || curtype === $TICK_BAD_PURE || curtype === $TICK_BAD_TAIL) {
  ...
  function parseValueTail(lexerFlags, valueFirstToken, assignable, isNewArg, leftHandSideExpression, astProp) {
    ...
    switch (curc) {
      ...
      case $$TICK_60:
        if (isTemplateStart(curtype)) return _parseValueTailTemplate(lexerFlags, valueFirstToken, assignable, isNewArg, astProp);
        break;
      case $$PLUS_2B:
  ...
  function _parseValueTailTemplate(lexerFlags, valueFirstToken, assignable, isNewArg, astProp) {
    ...
    let awaitYieldFlagsFromAssignable = ASSIGNABLE_UNDETERMINED;
    if (curtype === $TICK_PURE || curtype === $TICK_BAD_PURE) {
      parseQuasiPart(lexerFlags, IS_QUASI_TAIL, allowBadEscapesInTaggedTemplates);
    }
    else if (curtype === $TICK_HEAD || curtype === $TICK_BAD_HEAD) {
      ...
      if (isBadTickToken(curtype)) THROW('Template containd bad escape');
      THROW('Template should start as head or pure');
    }
  ...
  function parseArrayLiteralPattern(lexerFlagsBeforeParen, scoop, bindingType, skipInit, exportedNames, exportedBindings, _astProp) {
    ...
    AST_open(_astProp, 'ArrayExpression', arrayOpenToken);
    AST_set('elements', []);
  ...
  function parseObjectLikePatternSansAssign(outerLexerFlags, scoop, bindingType, isExpression, exportedNames, exportedBindings, astProp) {
    ...
    ASSERT(curc === $$CURLY_L_7B, 'should have asserted to be at the curly open of an objlit or pattern here...', curtok);
    ...
    if (isExpression === IS_EXPRESSION) {
      ...
      if (curtype === $EOF) THROW('Unexpected EOF while parsing an object literal or pattern');
      ASSERT_skipStatementStart('}', lexerFlags);
    }
  ...
  function parseObjectLikePart(lexerFlags, scoop, bindingType, exportedNames, exportedBindings, astProp) {
    ...
    else if (isNumberStringToken(curtype)) {
  ...
  function parseObjectPropertyValueAfterColon(lexerFlags, startOfKeyToken, keyToken, bindingType, assignableOnlyForYieldAwaitFlags, destructible, scoop,exportedNames, exportedBindings, astProp) {
    ...
    AST_open(astProp, NODE_NAME_PROPERTY, startOfKeyToken);
  ...
  function parseObjectLikePartFromIdent(lexerFlags, startOfPropToken, scoop, bindingType, exportedNames, exportedBindings, astProp) {
    ...
    if (curc === $$COMMA_2C || curc === $$CURLY_R_7D || curtok.str === '=') {
      ...
      if (curtype === $IDENT) {
        ...
        if (curc !== $$SQUARE_R_5D) THROW('Missing right square bracket for computed property, found `' + curtok.str + '` instead');
  ...
  function _parseClass(outerLexerFlags, originalOuterLexerFlags, scoop, isExpression) {
    ...
    if (curc !== $$CURLY_L_7B) THROW('Expected the opening curly `{` of a class body, found `' + curtok.str + '` instead');
  ...
  function _parseClassBody(lexerFlags, outerLexerFlags, originalOuterLexerFlags, scoop, bindingType, isExpression, exportedNames, exportedBindings, astProp) {
    ...
    if (isExpression === IS_EXPRESSION) {
      ...
      // - `class x {} /foo/`
      // - `class x {} 06`
      if (curtype === $EOF) THROW('Unexpected EOF while parsing a class body');
  ...
  function parseClassMethodIdentKey(lexerFlags, staticToken, asyncToken, starToken, getToken, setToken, astProp) {
    ...
    ASSERT(curtype === $IDENT, 'curtype is ident', curtok);
    ASSERT(staticToken === UNDEF_STATIC || staticToken.str === 'static', 'static token');
    ASSERT(asyncToken === UNDEF_ASYNC || asyncToken.str === 'async', 'async token');
    ASSERT(starToken === UNDEF_STAR || starToken.str === '*', 'gen token');
    ASSERT(getToken === UNDEF_GET || getToken.str === 'get', 'get token');
  ...
  function parseClassMethodComputedKey(lexerFlags, outerLexerFlags, staticToken, asyncToken, starToken, getToken, setToken, astProp) {
    ...
    // - `(fail = class A {[await](){}; "x"(){}}) => {}`
    if (curc !== $$SQUARE_R_5D) THROW('Missing right square bracket for computed member, found `' + curtok.str + '` instead');
    ASSERT_skipToParenOpen(']', lexerFlags);
...
function isTemplateStart(curtype) {
  return curtype === $TICK_PURE || curtype === $TICK_HEAD || curtype === $TICK_BAD_PURE || curtype === $TICK_BAD_HEAD;
}
