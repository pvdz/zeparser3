...
function ZeParser(code, goalMode = GOAL_SCRIPT, collectTokens = COLLECT_TOKENS_NONE, options = {}) {
  ...
  function parseBodyPartsWithDirectives(lexerFlags, scoop, labelSet, exportedNames, exportedBindings, paramsSimple, dupeParamErrorToken, functionNameTokenToVerify, isGlobalToplevel, fdState, astProp) {
    ...
    while (curtok.type !== $EOF && curc !== $$CURLY_R_7D) {
  ...
  function parseSemiOrAsi(lexerFlags) {
    ...
    if (curc === $$SEMI_3B) {
      ...
      if (curc === $$CURLY_R_7D || curtok.nl > 0 || curtok.type === $EOF) {
        tok_asi();
      } else {
    ...
    if (curtok.type === $IDENT) {
      ...
      switch (curtok.str) {
        ...
        case 'let':
          ...
          // [v]: `for (let x of y);`
          //                ^
          if (curtok.type === $IDENT || curc === $$SQUARE_L_5B || curc === $$CURLY_L_7B) {
            // [v]: `for (let x of y);`
            //                ^
            // [v]: `for (let [x] in y);`
            //                ^
  ...
  function parseReturnStatement(lexerFlags, astProp) {
    ...
    if (curtok.nl > 0 && isRegexToken(curtok.type)) {
      ...
      if (curtok.nl === 0 && curtok.type !== $EOF && curc !== $$SEMI_3B && curc !== $$CURLY_R_7D) {
  ...
  function parseSwitchCases(lexerFlags, scoop, labelSet, astProp) {
    ...
    while (true) {
      if (curtok.str === 'case') {
        ...
        ASSERT_skipStatementStart(':', lexerFlags);
        while (curtok.type !== $EOF && curc !== $$CURLY_R_7D && (curtok.type !== $IDENT || (curtok.str !== 'case' && curtok.str !== 'default'))) {
  ...
  function parseAnyVarDeclaration(lexerFlags, bindingToken, scoop, bindingType, bindingOrigin, exportedNames, exportedBindings, astProp) {
    ...
    if (curtok.type !== $IDENT && curc !== $$SQUARE_L_5B && curc !== $$CURLY_L_7B) THROW('Expected identifier, or array/object destructuring, next token is: ' + curtok);
  ...
  function parseObjectLikePart(lexerFlags, scoop, bindingType, exportedNames, exportedBindings, astProp) {
    ...
    if (curtok.type === $IDENT) {
      ...
      destructible |= subDestruct;
      ASSERT(curtok.type === $EOF || (curc !== $$COMMA_2C || hasAllFlags(subDestruct, CANT_DESTRUCT)), 'if comma then cannot destruct, should be dealt with in function');
      ASSERT(curtok.type === $EOF || (curc === $$COMMA_2C || curc === $$CURLY_R_7D), 'abstraction should parse whole rest/spread goal; ' + curtok);
    }
    else if (curc === $$SQUARE_L_5B) {
