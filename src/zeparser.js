...
function ZeParser(code, goalMode = GOAL_SCRIPT, collectTokens = COLLECT_TOKENS_NONE, options = {}) {
  ...
  function SCOPE_addBindingAndDedupe(scoop, name, bindingType, originIsVarDecl) {
    ASSERT(SCOPE_addBindingAndDedupe.length === arguments.length, 'arg count');
    return SCOPE_addBinding(scoop, name, bindingType, CHECK_DUPE_BINDS, originIsVarDecl);
  }
  function SCOPE_addBinding(scoop, name, bindingType, dupeChecks, originIsVarDecl) {
    ...
    if (bindingType === BINDING_TYPE_VAR) {
      ...
      if (dupeChecks === CHECK_DUPE_BINDS) {
        SCOPE_lexParentDupeCheck(lex, scoop.lexvar, hashed);
  ...
  function SCOPE_verifyLexical(scoop, skipParent) {
    let lex = scoop.lex;
    let lexvar = scoop.lexvar;
    for (let key in lex) {
      if (key[0] === '#' && key.length > 1) {
        ...
        if (!skipParent) SCOPE_lexParentDupeCheck(lex, lexvar, key);
      }
    }
    return false;
  }
  function SCOPE_lexParentDupeCheck(lex, lexvar, hashed) {
    ...
    */
    ...
    if (curtype === $IDENT) {
      ...
      if (isFuncDecl === IS_FUNC_DECL) {
        SCOPE_addBindingAndDedupe(outerScoop, name, nameBindingType, ORIGIN_IS_VAR_DECL);
      } else if (isFuncDecl === IS_FUNC_EXPR && isRealFuncExpr) {
        SCOPE_addBindingAndDedupe(innerScoop, name, nameBindingType, ORIGIN_IS_VAR_DECL);
      }
  ...
  function _parseClass(lexerFlags, scoop, optionalIdent, isExpression) {
    ...
    if (curtype === $IDENT && curtok.str !== 'extends') {
      ...
      SCOPE_addBindingAndDedupe(scoop, bindingName, BINDING_TYPE_LET, ORIGIN_NOT_VAR_DECL);
      AST_setIdent('id', curtok);
      ASSERT_skipAny($IDENT, lexerFlags);
  ...
  function parseConstStatement(lexerFlags, scoop, astProp) {
    ...
    if (SCOPE_verifyLexical(scoop)) THROW('Const binding attempted to get at least one name bound more than once');
    parseSemiOrAsi(lexerFlags);
  }
  ...
  function parseExportStatement(lexerFlags, scoop, exportedNames, astProp) {
    ...
    if (curc === $$D_64 && curtok.str === 'default') {
      ...
      if (curc === $$CURLY_L_7B) {
        ...
        if (SCOPE_verifyLexical(scoop)) THROW('Let export binding attempted to get at least one name bound more than once');
        AST_set('source', null);
      } else if (curc === $$C_63) {
        if (curtok.str === 'const') {
          ...
          if (SCOPE_verifyLexical(scoop)) THROW('Const export binding attempted to get at least one name bound more than once');
    ...
    if (curtype === $IDENT) {
      switch (curtok.str) {
        ...
        case 'let':
          ...
          if (curtype === $IDENT || curc === $$SQUARE_L_5B || curc === $$CURLY_L_7B) {
            if (curtok.str === 'in') {
              ...
              if (SCOPE_verifyLexical(scoop)) THROW('Let for-binding attempted to get at least one name bound more than once');
            }
        ...
        case 'const':
          ...
          if (SCOPE_verifyLexical(scoop)) THROW('Const for-binding attempted to get at least one name bound more than once');
          assignable = IS_ASSIGNABLE; // i think.
          break;
  ...
  function parseImportDefault(lexerFlags, scoop) {
    ...
    SCOPE_addBindingAndDedupe(scoop, curtok.str, BINDING_TYPE_CONST, ORIGIN_NOT_VAR_DECL);
  ...
  function parseImportObject(lexerFlags, scoop) {
    ...
    while (curtype === $IDENT) {
      ...
      if (curtok.str === 'as') {
        ...
        SCOPE_addBindingAndDedupe(scoop, nameToken.str, BINDING_TYPE_CONST, ORIGIN_NOT_VAR_DECL);
        AST_setIdent('local', nameToken);
      }
  ...
  function parseLetDeclaration(lexerFlags, scoop, labelSet, astProp) {
    ...
    if (curtype === $IDENT || curc === $$SQUARE_L_5B || curc === $$CURLY_L_7B) {
      ...
      if (SCOPE_verifyLexical(scoop)) THROW('Let binding attempted to get at least one name bound more than once');
      parseSemiOrAsi(lexerFlags);
    } else if (hasAllFlags(lexerFlags, LF_STRICT_MODE)) {
  ...
  function parseTryStatement(lexerFlags, scoop, labelSet, astProp) {
    ...
    if (curc === $$C_63 && curtok.str === 'catch') {
      ...
      if (SCOPE_verifyLexical(catchHeadScoop, true)) THROW('Catch binding had at least one duplicate name bound');
  ...
  function parseBinding(lexerFlags, scoop, bindingType, bindingOrigin, defaultsOption, dupeChecks, exportedNames, astProp) {
    ...
    if (curtype === $IDENT) {
      ...
      let destructible = parseObjectLiteralPatternAndAssign(lexerFlags, scoop, bindingType, SKIP_INIT, NOT_CLASS_METHOD, exportedNames, astProp);
  ...
  function parseExpressionAfterIdent(lexerFlags, identToken, allowAssignment, astProp) {
    let assignable = parseValueAfterIdent(lexerFlags, identToken, NOT_NEW_TARGET, allowAssignment, astProp);
  ...
  function parseValueHeadBody(lexerFlags, maybe, checkNewTarget, allowAssignment, astProp) {
    ...
    if (curtype === $IDENT) {
      ...
      if (curc === $$CURLY_L_7B) {
        let wasDestruct = parseObjectLiteralPatternAndAssign(lexerFlags, DO_NOT_BIND, BINDING_TYPE_NONE, PARSE_INIT, NOT_CLASS_METHOD, undefined, astProp);
  ...
  function parseValueHeadBodyAfterIdent(lexerFlags, identToken, checkNewTarget, allowAssignment, astProp) {
    ...
    ASSERT(arguments.length === parseValueHeadBodyAfterIdent.length, 'expecting args');
    // for new only a subset is accepted;
    // - super
    // - metaproprety
    // - this
  ...
  function parseAfterVarName(lexerFlags, identToken, assignable, allowAssignment, astProp) {
    ASSERT(arguments.length === parseAfterVarName.length, 'arg count');
  ...
  function parseArrowParenlessFromPunc(lexerFlags, identToken, astProp) {
    ...
    SCOPE_addBindingAndDedupe(scoop, identToken.str, BINDING_TYPE_ARG, ORIGIN_NOT_VAR_DECL);
  ...
  function _parseGroupToplevels(lexerFlagsBeforeParen, asyncKeywordPrefixed, asyncStmtOrExpr, allowAssignment, lhpToken, isDeleteArg, astProp) {
    ...
    while (curc !== $$PAREN_R_29) { // top-level group loop, list of ident, array, object, rest, and other expressions
      if (curtype === $IDENT) {
        ...
        if (curtok.str === '=') {
          ...
          switch (identToken.str) {
            ...
            default:
              ...
              assignable = bindingAssignableIdentCheck(identToken, BINDING_TYPE_NONE, lexerFlags);
              SCOPE_addBinding(scoop, identToken.str, BINDING_TYPE_ARG, SKIP_DUPE_CHECKS, ORIGIN_NOT_VAR_DECL);
        ...
        simpleArgs = ARGS_COMPLEX;
        destructible |= parseObjectLiteralPatternAndAssign(lexerFlags, scoop, BINDING_TYPE_NONE, PARSE_INIT, NOT_CLASS_METHOD, undefined, astProp);
    ...
    if (curtok.str === '=>') {
      ...
      if (SCOPE_verifyLexical(scoop, true)) THROW('Arrow had at least one duplicate parameter name bound');
      ...
      if (curtype === $IDENT) {
        ...
        if (curtok.str === '=') {
          ...
          if (assignable === NOT_ASSIGNABLE) destructible |= CANT_DESTRUCT;
            ...
            SCOPE_addBinding(scoop, identToken.str, bindingType, SKIP_DUPE_CHECKS, ORIGIN_NOT_VAR_DECL);
          ...
          let assignable = parseValueHeadBodyAfterIdent(lexerFlags, identToken, NOT_NEW_TARGET, ALLOW_ASSIGNMENT, astProp);
        ...
        // - [{}.foo] = x
        // - [{}[foo]] = x
        let nowDestruct = parseObjectLiteralPatternAndAssign(lexerFlags, scoop, bindingType, PARSE_INIT, NOT_CLASS_METHOD, exportedNames, astProp);
  ...
  function parseObjectLiteralPatternAndAssign(lexerFlags, scoop, bindingType, skipInit, isClassMethod, exportedNames, _astProp) {
    ...
    AST_set('properties', []);
    let destructible = parseObjectLikePatternSansAssign(lexerFlags | LF_NO_ASI, scoop, bindingType, isClassMethod, IS_EXPRESSION, exportedNames, 'properties');
  ...
  function parseClassbody(lexerFlags, scoop, bindingType, isExpression, astProp) {
    ...
    AST_set('body', []);
    parseObjectLikePatternSansAssign(lexerFlags, scoop, bindingType, IS_CLASS_METHOD, isExpression, undefined, 'body');
    AST_close('ClassBody');
  }
  function parseObjectLikePatternSansAssign(_lexerFlags, scoop, bindingType, isClassMethod, isExpression, exportedNames, astProp) {
    ...
    if (curtype === $IDENT) {
      ...
      if (curc === $$COLON_3A) {
        ...
        if (curtype === $IDENT) {
          ...
          switch (nameBinding.str) {
            ...
            case 'yield':
              ...
              if (yieldAssignable === NOT_ASSIGNABLE) destructible |= CANT_DESTRUCT;
                ...
                SCOPE_addBinding(scoop, nameBinding.str, bindingType, SKIP_DUPE_CHECKS, ORIGIN_NOT_VAR_DECL);
          ...
          AST_set('method', false);
          AST_set('computed', false);
          destructible |= parseObjectLiteralPatternAndAssign(lexerFlags, scoop, bindingType, PARSE_INIT, NOT_CLASS_METHOD, exportedNames, 'value');
    ...
    if (curc === $$COMMA_2C || curc === $$CURLY_R_7D || curtok.str === '=') {
      ...
      if (curtype === $IDENT) {
        ...
        if (identToken.str === 'yield') {
          ...
          let assignable = parseValueAfterIdent(lexerFlags, identToken, NOT_NEW_ARG, ALLOW_ASSIGNMENT, 'value');
  ...
  function _parseArrowableSpreadOrRest(lexerFlags, scoop, closingCharOrd, bindingType, groupTopLevel, asyncIdent, exportedNames, astProp) {
    ...
    if (curtype === $IDENT) {
      ...
      // - `[...{x}.foo] = x`
      // (and object)
      destructible = parseObjectLiteralPatternAndAssign(lexerFlags, scoop, bindingType, SKIP_INIT, NOT_CLASS_METHOD, exportedNames, astProp);
      assignable = hasAllFlags(destructible, CANT_DESTRUCT) ? NOT_ASSIGNABLE : IS_ASSIGNABLE; // this is valid: `[...{x}=y];`
