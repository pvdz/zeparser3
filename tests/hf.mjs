...
(async function(){
  let file = USE_BUILD ? ZEPARSER_PROD_FILE : ZEPARSER_DEV_FILE;
  console.log('Running test on', BOLD + file + RESET);
  let zeparser = (await import(file)).default;

  let parse = (code, testVariant) => testZeParser(zeparser, code, testVariant, true);
