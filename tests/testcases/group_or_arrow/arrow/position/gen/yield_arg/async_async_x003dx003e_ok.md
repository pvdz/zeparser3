...
- Path: tests/testcases/group_or_arrow/arrow/position/gen/yield_arg/async_async_x003dx003e_ok.md
...
tokens (14x):
       ...
       PUNC_CURLY_OPEN ID_yield ID_async ID_async PUNC_EQ_GT IDENT ASI
       PUNC_CURLY_CLOSE
`````
...
````js
function* f() {(yield (async async => (ok)));}
````
