...
- Path: tests/testcases/let_declaration/binding_generic/let_newline_ident/gen/newline_in_async/protected.md
...
tokens (11x):
       ...
       PUNC_CURLY_OPEN ID_let ID_protected ASI PUNC_CURLY_CLOSE
`````
...
`````
throws: Parser error!
  Attempted to create a `let` binding on special reserved keyword `protected` but: Cannot use this reserved word as a variable name in strict mode
...
````js
async function f() {let protected;}
````
